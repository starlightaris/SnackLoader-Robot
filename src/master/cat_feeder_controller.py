...
def rtdb_loop():
    ...
    while True:
        ...
        # 1. IMMEDIATE DOG OVERRIDE (Safety)
        ...
        # 2. CAT FSM
            ...
            # If cat is seen, reset the 10m timer
            if cat_detected:
                timeout_ts = now + 600
            
            # If 10m passed and cat is gone, close
            if now > timeout_ts and not cat_detected:
                print("Timeout reached -> closing lid")
                send_serial("CLOSE_LID")
                lid_open = False
                fsm_state = "IDLE"
